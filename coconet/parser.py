...
def parse_args():
    ...
    parser.add_argument('-fl', '--fragment-length', type=int, default=1024,
                        help='Length of contig fragments in bp')
    parser.add_argument('-t', '--threads', type=int, default=20,
                        help='Number of threads')
    parser.add_argument('--features', type=str, default=['coverage', 'composition'],
    ...
    io_group.add_argument('--fasta', type=str, action=ToPathAction, required=True,
                          help='Path to your assembly file (fasta formatted)')
    io_group.add_argument('--bam', type=str, nargs='+', action=ToPathAction, required=True,
                          help='List of paths to your coverage files (bam formatted)')
    io_group.add_argument('--h5', type=str, nargs='+', action=ToPathAction, required=True,
                          help='Experimental: coverage in hdf5 format (keys are contigs, values are (sample, contig_len) arrays')
    io_group.add_argument('--output', type=str, default='output', action=ToPathAction,
                          help='Path to output directory')
    io_group.add_argument('--verbosity', type=str, default='INFO',
                          choices=['ERROR', 'WARNING', 'INFO', 'DEBUG'],
                          help='Verbosity level')

    preproc_group = parser.add_argument_group(title='Preprocessing')
    preproc_group.add_argument('--min-ctg-len', type=int, default=-1,
    ...
    frag_group.add_argument('--fragment-step', type=int, default=128,
                            help='Fragments spacing')
    frag_group.add_argument('--test-ratio', type=float, default=0.1,
                            help='Ratio for train / test split')
    frag_group.add_argument('--n-train', type=int, default=int(1e6),
                            help='Number of training examples')
    frag_group.add_argument('--n-test', type=int, default=int(1e4),
                            help='Number of test examples')

    dl_group = parser.add_argument_group(title='Neural network')
    dl_group.add_argument('--batch-size', type=int, default=256,
                          help='Batch size for training')
    dl_group.add_argument('--learning-rate', type=float, default=1e-4,
                          help='Learning rate for gradient descent')
    dl_group.add_argument('--load-batch', type=int, default=200,
                          help='Number of coverage batch to load in memory. Consider lowering this value if your RAM is limited.')
    dl_group.add_argument('--compo-neurons', type=int, default=[64, 32], nargs=2,
                          help='Number of neurons for the composition dense layers (x2)')
    dl_group.add_argument('--cover-neurons', type=int, default=[64, 32], nargs=2,
                          help='Number of neurons for the coverage dense layers (x2)')
    dl_group.add_argument('--cover-filters', type=int, default=32,
                          help='Number of filters for convolution layer of coverage network.')
    dl_group.add_argument('--cover-kernel', type=int, default=7,
                          help='Kernel size for convolution layer of coverage network.')
    dl_group.add_argument('--cover-stride', type=int, default=3,
                          help='Convolution stride for convolution layer of coverage network.')
    dl_group.add_argument('--merge-neurons', type=int, default=32,
                          help='Number of neurons for the merging layer (x1)')
    dl_group.add_argument('--norm', action='store_true', default=False,
                          help='Normalize the k-mer occurrences to frequencies')
    dl_group.add_argument('-k', '--kmer', type=int, default=4,
                          help='k-mer size for composition vector')
    dl_group.add_argument('--no-rc', action='store_true', default=False,
                          help='Do not add the reverse complement k-mer occurrences to the composition vector.')
    dl_group.add_argument('--wsize', type=int, default=64,
                          help='Smoothing window size for coverage vector')
    dl_group.add_argument('--wstep', type=int, default=32,
                          help='Subsampling step for coverage vector')
    dl_group.add_argument('--n-frags', type=int, default=30,
    ...
    cluster_group.add_argument('--max-neighbors', type=int, default=100,
                               help='Maximum number of neighbors to consider to compute the adjacency matrix.')
    cluster_group.add_argument('--theta', type=float, default=0.8,
                               help='Minimum percent of edges between two contigs to form an edge between them')
    cluster_group.add_argument('--gamma1', type=float, default=0.1,
