...
- The run configuration `config.yaml` with the parameters
- The filtered alignments (both .bam and .h5 formats) and the filtered assembly (.fasta)
- The `pytorch` deep learning model, `CoCoNet.pth`
- The model results on the test contigs, `CoCoNet_test.csv`
