...
def cluster(model, config):
    ...
    if not os.path.exists(config.outputs['clustering']['adjacency_matrix']):
        ...
        adjacency_matrix = np.identity(n_contigs)*(n_frags**2+1) - np.ones((n_contigs, n_contigs))
    ...
    for algo in config.clustering['algo'].split(','):
        if algo == 'louvain':
            ...
            leiden_generator = leidenalg.find_partition(G, leidenalg.ModularityVertexPartition)
            communities = (pd.Series(dict(enumerate(leiden_generator)))
                           .explode()
                           .sort_values()
                           .index)
        ...
        assignments.to_csv("{}/{}_nf{}.csv"
                           .format(config.outdir, algo, config.clustering['n_frags']))

def refine_clusters(assignments, model, config):
    handles = {key: h5py.File(filename) for key, filename in config.outputs['repr'].items()}

    contigs = np.array(list(handles['coverage'].keys()))    
    clusters = (assignments.reset_index()
                .groupby('clusters', as_index=False)
                .agg(list)[['clusters', 'index']])
    adj = np.load(config.outputs['clustering']['adjacency_matrix'])

    for c, indices in clusters.values:
        indices1, indices2 = tuple(zip(*[(i, j)
